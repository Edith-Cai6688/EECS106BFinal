...
class OperationalSpaceControllerPID(Controller):
    """
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        self.integral_error_ori = np.zeros(3)
        self.ki = self.nums2array(kwargs.get("ki", 0.0), 6)
        self.i_limit = kwargs.get("i_limit", 100)

        self.kdarr = self.nums2array(kwargs.get("kdarr", 0.0), 6)
        self.kparr = self.nums2array(kwargs.get("kparr", 0.0), 6)
    ...
    def run_controller(self):
        ...
        self.integral_error_pos += position_error / self.control_freq
        self.integral_error_pos = np.clip(self.integral_error_pos, -self.i_limit, self.i_limit)

        # F_r = kp * pos_err + kd * vel_err
        position_term = np.multiply(np.array(position_error), np.array(self.kparr[0:3]))
        derivative_term = np.multiply(vel_pos_error, self.kdarr[0:3])
        integral_term = np.multiply(self.ki[0:3], self.integral_error_pos)
        # desired_force = np.multiply(np.array(position_error), np.array(self.kp[0:3])) + np.multiply(
        #     vel_pos_error, self.kd[0:3]) +
        #  np.multiply(self.ki[0:3], self.integral_error_pos)
        desired_force = position_term + derivative_term + integral_term
        # print(f"POSITION TERM: {position_term}\nDERIVATIVE_TERM: {derivative_term}\nINTEGRAL_TERM: {integral_term}")
        ...
        self.integral_error_ori += ori_error / self.control_freq
        self.integral_error_ori = np.clip(self.integral_error_ori, -self.i_limit, self.i_limit)

        # Tau_r = kp * ori_err + kd * vel_err
        desired_torque = np.multiply(np.array(ori_error), np.array(self.kparr[3:6])) + np.multiply(
            vel_ori_error, self.kdarr[3:6]
        ) + self.ki[3:6] * self.integral_error_ori
        # print(f"{self.ki}, {self.integral_error_ori}, {self.ki[3:6]*self.integral_error_ori}")
